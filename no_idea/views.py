...
def catlist(request, category_id):
    cat_list = art_gallery.objects.filter(category_id=category_id, published=True)
    return render(request, 'catlist.html', {"cat_list":cat_list})

def home(request):
    data_1 = category.objects.all()
...
def engine(request, user_id):
    if (request.user.is_staff or request.user.is_superuser):
        ...
        if request.method=="POST":
            ...
            postForm = my_engine()
            context = {"postForm":postForm, "my_list_1":my_list_1}
    else:
        raise Http404
...
def eachpost(request, slug, category_id):
    cat_list_2 = art_gallery.objects.filter(category_id=category_id, published=True)
    instance = get_object_or_404(art_gallery, slug=slug, category_id=category_id)
